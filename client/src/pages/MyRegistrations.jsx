...
export default function MyRegistrations() {
  ...
  return (
    <PageLayout>
      <div className="w-full sm:w-[90%] lg:w-[80%] mx-auto p-4 sm:p-6 transition-colors duration-300 bg-white text-gray-800 dark:bg-gray-800 dark:text-gray-200">
        <h2 className="text-2xl sm:text-3xl font-bold mb-6">Your Registrations</h2>
        {isLoading ? (
          <Spinner />
        ) : (
          <>
                  ...
                  <div className="overflow-x-auto rounded-md">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                          {['Event', 'Date', 'Total Charges', 'Payment ID', 'Status', 'Actions'].map((header) => (
                            <th
                              key={header}
                              scope="col"
                      ...
                      <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        {filteredRegistrations.map((registration) => (
                          <tr key={registration._id}>
                            <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-200">
                            ...
                            <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                              <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100">
                                {registration.currentStatus.charAt(0).toUpperCase() + registration.currentStatus.slice(1)}
                              </span>
                            </td>
